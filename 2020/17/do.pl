...
sub printgrid($) {
	...
	for (my $z=0; $z<@${grid}; $z++) {
		print "z=$z\n  ";
		for (my $y=0; $y<@{$grid->[$z]}; $y++) {
			for (my $x=0; $x<@{$grid->[$z]->[$y]}; $x++) {
				print $grid->[$z]->[$y]->[$x];
			}
			print "\n  ";
		}
...
sub count_neighbours {
	my ($cx,$cy,$cz) = (shift,shift,shift);
	my $grid = shift or die;

	my $z_num = scalar @{$grid};
	my $y_num = scalar @{$grid->[0]};
	my $x_num = scalar @{$grid->[0]->[0]};
	...
	for (my $z=$cz-1; $z<=$cz+1; $z++) {
		...
		for (my $y=$cy-1; $y<=$cy+1; $y++) {
			...
			for (my $x=$cx-1; $x<=$cx+1; $x++) {
				...
				next if $z==$cz and $y==$cy and $x==$cx;
				#printf(" --> ($x,$y,$z)==%s\n",$grid->[$z]->[$y]->[$x]) if $debug;
	...
	for (my $new_z=-1; $new_z<=$z_num; $new_z++) {
		...
		for (my $new_y=-1; $new_y<=$y_num; $new_y++) {
			...
			for (my $new_x=-1; $new_x<=$x_num; $new_x++) {
				...
				my $count = count_neighbours($new_x,$new_y,$new_z,$grid);


				my $cell = '_';
				if (defined($old_z) and defined($old_y) and defined($old_x)) {
					$cell = $grid->[$old_z]->[$old_y]->[$old_x];
				}
				...
				$newgrid[$new_z+1]->[$new_y+1]->[$new_x+1] = $newcell;
			}
		}
	}
