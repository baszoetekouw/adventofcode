...
sub count_neighbours($$$$$) {
	...
	my $debug = 0;
	$debug = 1 if $cx==0 and $cy==0 and $cz==0 and $cw==0;
	...
	#print "Debug: counted $num neighbouring cells\n";
	#if ($num==1) {print "found 1 neighbour for ($cx,$cy,$cx,$cw)\n";}
	return $count;
}
			...
			for (my $new_y=-1; $new_y<=$y_num; $new_y++) {
				...
				for (my $new_x=-1; $new_x<=$x_num; $new_x++) {
					...
					if (defined($old_w) and defined($old_z) and defined($old_y) and defined($old_x)) {
						$cell = $grid->[$old_z]->[$old_z]->[$old_y]->[$old_x];
					}
